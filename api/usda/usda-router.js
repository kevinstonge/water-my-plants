  ...
  try {
    if (req.query && req.query.commonName) {
      if (req.query.commonName.length > 3) {
        ...
        const { results, pagination } = await Usda.searchByCommonName({
          query: req.query.commonName,
          offset,
          limit,
        });
        ...
        res
          .status(400)
          .json({ error: "query must contain at least 4 characters" });
      }
    } else {
      res
        .status(400)
        .json({ error: "missing query: /search?commonName=query" });
    }
  } catch (error) {
    console.log(error);
...
router.get("/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "error getting information about that plant" });
  }
});
